...
Фреймворк создан в целях обучения, но предлагает некоторые продвинутые опции, такие как гибкая [система роутинга](#система-роутинга) и [экшен-фильтры](#экшен-фильтры). Основой для [представлений](#представления) является Twig.
...
[Роутер](src/Service/Router.php) распознает запрошенный URL и вызывает соответствующий экшн (метод) определенного контроллера. Маршруты добавляются в [фронт контроллер](public/index.php). 
...
Доступно добавление поддиректорий для организации контроллеров, для этого при добавлении роута необходимо указать и неймспейс (описано в секции [Система роутинга](#система-роутинга))
...
- Переработка в сторону DI класса `BaseModel`. Статические методы в нынеших моделях не лучшим образом тестируются.
